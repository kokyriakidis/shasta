...
//     uint64_t homopolymerContextLength = 12,  // HPC_PL
//     uint64_t maxRepeatLength = 4,            // HPC_RR
//     uint64_t homopolymerCutoff = 2           // HPC_CC
// ) {
//     const uint64_t readLength = readSequence.baseCount;
//     SHASTA_ASSERT(sitePositionInRead < readLength);

//     // Use int64_t for all position variables to match hifiasm exactly
//     int64_t leftBoundary = (sitePositionInRead >= homopolymerContextLength)
//         ? static_cast<int64_t>(sitePositionInRead) - homopolymerContextLength
//         : 0;
//     int64_t rightBoundary = (sitePositionInRead + homopolymerContextLength < readLength)
//         ? static_cast<int64_t>(sitePositionInRead) + homopolymerContextLength
//         : static_cast<int64_t>(readLength - 1);

//     for (uint64_t repeatLength = 1; repeatLength <= maxRepeatLength; ++repeatLength) {
//         uint64_t minHomopolymerLength = repeatLength * homopolymerCutoff;

//         // Pattern 1: Including site (forward and backward)
//         int64_t forwardEnd1 = static_cast<int64_t>(sitePositionInRead) + repeatLength;
//         while (
//             forwardEnd1 < rightBoundary &&
//             (forwardEnd1 - static_cast<int64_t>(repeatLength)) >= leftBoundary &&
//             readSequence[forwardEnd1].value == readSequence[forwardEnd1 - repeatLength].value
//         ) {
//             ++forwardEnd1;
//         }
//         int64_t regionEnd1 = (forwardEnd1 > rightBoundary) ? rightBoundary : forwardEnd1;

//         int64_t backwardStart1 = static_cast<int64_t>(sitePositionInRead) - 1;
//         while (
//             backwardStart1 >= leftBoundary &&
//             (backwardStart1 + static_cast<int64_t>(repeatLength)) < rightBoundary &&
//             readSequence[backwardStart1].value == readSequence[backwardStart1 + repeatLength].value
//         ) {
//             --backwardStart1;
//         }
//         int64_t regionStart1 = backwardStart1 + 1;
//         if (regionStart1 < leftBoundary) regionStart1 = leftBoundary;

//         if ((regionEnd1 - regionStart1) > static_cast<int64_t>(repeatLength) &&
//             (regionEnd1 - regionStart1) >= static_cast<int64_t>(minHomopolymerLength)) {
//             return true;
//         }

//         // Pattern 2: Excluding site (forward only)
//         int64_t forwardEnd2 = static_cast<int64_t>(sitePositionInRead) + repeatLength + 1;
//         while (
//             forwardEnd2 < rightBoundary &&
//             (forwardEnd2 - static_cast<int64_t>(repeatLength)) >= leftBoundary &&
//             readSequence[forwardEnd2].value == readSequence[forwardEnd2 - repeatLength].value
//         ) {
//             ++forwardEnd2;
//         }
//         int64_t regionEnd2 = (forwardEnd2 > rightBoundary) ? rightBoundary : forwardEnd2;

//         int64_t regionStart2 = static_cast<int64_t>(sitePositionInRead) + 1;
//         if (regionStart2 < leftBoundary) regionStart2 = leftBoundary;

//         if ((regionEnd2 - regionStart2) > static_cast<int64_t>(repeatLength) &&
//             (regionEnd2 - regionStart2) >= static_cast<int64_t>(minHomopolymerLength)) {
//             return true;
//         }

//         // Pattern 3: Including site (backward and forward)
//         int64_t backwardStart3 = static_cast<int64_t>(sitePositionInRead) - repeatLength;
//         while (
//             backwardStart3 >= leftBoundary &&
//             (backwardStart3 + static_cast<int64_t>(repeatLength)) < rightBoundary &&
//             readSequence[backwardStart3].value == readSequence[backwardStart3 + repeatLength].value
//         ) {
//             --backwardStart3;
//         }
//         int64_t regionStart3 = backwardStart3 + 1;
//         if (regionStart3 < leftBoundary) regionStart3 = leftBoundary;

//         int64_t forwardEnd3 = static_cast<int64_t>(sitePositionInRead) + 1;
//         while (
//             forwardEnd3 < rightBoundary &&
//             (forwardEnd3 - static_cast<int64_t>(repeatLength)) >= leftBoundary &&
//             readSequence[forwardEnd3].value == readSequence[forwardEnd3 - repeatLength].value
//         ) {
//             ++forwardEnd3;
//         }
//         int64_t regionEnd3 = (forwardEnd3 > rightBoundary) ? rightBoundary : forwardEnd3;

//         if ((regionEnd3 - regionStart3) > static_cast<int64_t>(repeatLength) &&
//             (regionEnd3 - regionStart3) >= static_cast<int64_t>(minHomopolymerLength)) {
//             return true;
//         }

//         // Pattern 4: Excluding site (backward only)
//         int64_t backwardStart4 = static_cast<int64_t>(sitePositionInRead) - repeatLength - 1;
//         while (
//             backwardStart4 >= leftBoundary &&
//             (backwardStart4 + static_cast<int64_t>(repeatLength)) < rightBoundary &&
//             readSequence[backwardStart4].value == readSequence[backwardStart4 + repeatLength].value
//         ) {
//             --backwardStart4;
//         }
//         int64_t regionStart4 = backwardStart4 + 1;
//         if (regionStart4 < leftBoundary) regionStart4 = leftBoundary;

//         int64_t regionEnd4 = static_cast<int64_t>(sitePositionInRead);
//         if (regionEnd4 > rightBoundary) regionEnd4 = rightBoundary;

//         if ((regionEnd4 - regionStart4) > static_cast<int64_t>(repeatLength) &&
//             (regionEnd4 - regionStart4) >= static_cast<int64_t>(minHomopolymerLength)) {
//             return true;
//         }
//     }
//     return false;
...
// bool isSiteInHomopolymerRegionSimple(
//     uint64_t sitePositionInRead,
//     const shasta::LongBaseSequenceView& readSequence,
...
//         // 0 = Match Evidence, 1 = SNP Evidence, 2 = Match Evidence In Homopolymer Region, 3 = SNP Evidence In Homopolymer Region
//         // hetBase1Type will be set to 0 or 2 because by convention the target read will have hetBase1.
//         uint64_t hetBase1Type = 0;
...
//         // 0 = Match Evidence, 1 = SNP Evidence, 2 = Match Evidence In Homopolymer Region, 3 = SNP Evidence In Homopolymer Region
//         // hetBase2Type will be set to 1 or 3 because by convention the read with the SNP will have hetBase2.
//         uint64_t hetBase2Type = 0;

//         uint64_t totalNumberOfOtherEvidence = -1;
//         uint64_t het1ReverseStrandCount = -1;
//         int64_t snpScore = -1;
...
//     vector<vector<bool> > threadReadsWithNoHetSites;
...
//         threadReadsWithNoHetSites.resize(threadCount);
...
//             threadReadsWithNoHetSites[i].resize(readCount, false);
//         }
...
//     vector<bool>& readsWithNoHetSites = phasingThreadDataReadGraph5->threadReadsWithNoHetSites[threadId];
...
//                 const auto orientedReadMarkers0 = markers[currentOrientedReadId0.getValue()];
//                 const uint32_t positionStart0 = orientedReadMarkers0[alignmentInfo.data[0].firstOrdinal].position;
//                 const uint32_t positionEnd0 = orientedReadMarkers0[alignmentInfo.data[0].lastOrdinal].position + uint32_t(assembler.assemblerInfo->k);

//                 // const auto orientedReadMarkers1 = markers[currentOrientedReadId1.getValue()];
//                 // const uint32_t positionStart1 = orientedReadMarkers1[alignmentInfo.data[1].firstOrdinal].position;
//                 // const uint32_t positionEnd1 = orientedReadMarkers1[alignmentInfo.data[1].lastOrdinal].position + uint32_t(assembler.assemblerInfo->k);

//                 // const uint64_t readLength0 = assembler.getReads().getRead(currentOrientedReadId0.getReadId()).baseCount;
//                 // const uint64_t readLength1 = assembler.getReads().getRead(currentOrientedReadId1.getReadId()).baseCount;
...
//                 // debugOut << "Thread ID: " << threadId << " Trying to add interval [" << positionStart0 << ", " << positionEnd0 << ") for alignment " << alignmentId << " (Other Read: " << currentOrientedReadId1 << ")" << endl;
//                 // debugOut << "Thread ID: " << threadId << " Left trim 0: " << leftTrimBases0 << " Right trim 0: " << rightTrimBases0 << endl;
//                 // debugOut << "Thread ID: " << threadId << " Left trim 1: " << leftTrimBases1 << " Right trim 1: " << rightTrimBases1 << endl;
//                 // debugOut << "Thread ID: " << threadId << " Read length 0: " << readLength0 << " Read length 1: " << readLength1 << endl;

//                 // --- Add interval to the map ---
//                 if (positionEnd0 > positionStart0) { // Ensure a valid interval
//                     Interval interval = Interval::closed(positionStart0, positionEnd0); // Create a [positionStart0, positionEnd0] interval
...
//                 } else {
//                     debugOut << "Thread ID: " << threadId << " Skipping invalid interval [" << positionStart0 << ", " << positionEnd0 << "] for alignment " << alignmentId << endl;
//                 }
//             }
...
//             cout << timestamp << "Thread ID: " << threadId << " Finished looping over alignments for read ID: " << readId0 << endl;
...
//                 uint64_t kmerStart = (positionInRead0 >= 12) ? positionInRead0 - 12 : 0;
//                 uint64_t kmerLength = min(25UL, readId0Sequence.baseCount - kmerStart);
//                 extractKmer(readId0Sequence, kmerStart, kmerLength, kmer);
                
//                 uint64_t homopolymerContextLength = 12;
//                 uint64_t maxRepeatLength = 4;
//                 uint64_t homopolymerCutoff = 2;
//                 // bool siteIsInHomopolymerRegion = isSiteInHomopolymerRegionReadGraph5(positionInRead0, readId0Sequence, homopolymerContextLength, maxRepeatLength, homopolymerCutoff);
...
//                 //     //continue;
//                 // }
//                 bool siteIsInHomopolymerRegion = false;
...
//                 // If the most frequent base (highestCoverageBaseCounts) is a gap, then skip this site.
//                 // This is because we cannot phase gaps, and they are not informative for phasing.
//                 if (baseCounts[0].second == 4) {
//                     continue;
//                 }
                
//                 // Check if the highest coverage variation base has coverage 3 or more (minimumNumberOfAlignmentsSupportingAChangeFromTheTargetRead)
//                 if (highestCoverageBaseCounts >= minimumNumberOfAlignmentsSupportingAChangeFromTheTargetRead) {
...
//                     // Filter the site if there are two alternative alleles with the same coverage
//                     if (highestCoverageBaseCounts == secondHighestCoverageBaseCounts) {
//                         continue;
//                     }
...
//                     cout << "positionInRead0: " << positionInRead0 << " is a potential heterozygous site." << endl;
...
//                         totalCoverageInThisPosition = coveringAlignmentInfo.size() + 1;
...
//                     // Calculate the coverage of the target read in this position
//                     uint64_t coverageOfTheBaseOfTheTargetRead = totalCoverageInThisPosition - totalNumberOfAlignmentsSupportingAChangeFromTheTargetRead;

//                     // This is the minimum needed number of alignments that
//                     // support the target read in that position. 
//                     // This is 3 + 1(the target read) = 4
//                     uint64_t minimumNumberOfAlignmentsSupportingTheTargetRead = 4;

//                     // Skip this position if the coverage supporting the target read is less than 4 (including the target read)
//                     if (coverageOfTheBaseOfTheTargetRead < minimumNumberOfAlignmentsSupportingTheTargetRead) {
//                         sitesSkippedDueToInsufficientCoverage++;
...
//                     cout << "Position: " << positionInRead0 << " A: " << totalNumberOfA << " C: " << totalNumberOfC << " G: " << totalNumberOfG << " T: " << totalNumberOfT << " Gap: " << totalNumberOfGap << " Total Alignments that support a change: " << totalNumberOfAlignmentsWithoutGapsSupportingAChangeFromTheTargetRead << ". Total alignments: " << totalCoverageInThisPosition << "." << endl;
                    


//                     // 95% of the reads need to be informative
//                     double filterAvailable1 = coverageOfTheBaseOfTheTargetRead + highestCoverageBaseCounts;
//                     double filterThreshold1 = 0.95;
//                     filterAvailable1 = filterAvailable1 / (double)totalCoverageInThisPosition;
//                     if (filterAvailable1 <= filterThreshold1) {
//                         continue;
//                     }

//                     // At least 70% of the errors need to be supporting the highest coverage alternate base
//                     double filterAvailable2 = highestCoverageBaseCounts;
//                     double filterThreshold2 = 0.70;
//                     filterAvailable2 = filterAvailable2 / (double)totalNumberOfAlignmentsSupportingAChangeFromTheTargetRead;
//                     if (filterAvailable2 <= filterThreshold2) {
//                         continue;
//                     }

//                     // // Filter out sites with low positional allele coverage
//                     // const double minPositionalAlleleCoverage = min(coverageOfTheBaseOfTheTargetRead, highestCoverageBaseCounts);
//                     // double filterThreshold = 0.35;
//                     // double filterAvailable = minPositionalAlleleCoverage / (double)totalCoverageInThisPosition;
//                     // if (filterAvailable <= filterThreshold || totalCoverageInThisPosition < 10) {
//                     //     continue;
//                     // }
                

//                     cout << "totalCoverageInThisPosition: " << totalCoverageInThisPosition << endl;
//                     cout << "coverageOfTheBaseOfTheTargetRead: " << coverageOfTheBaseOfTheTargetRead << endl;
//                     cout << "highestCoverageBaseCounts: " << highestCoverageBaseCounts << endl;
//                     cout << "totalNumberOfAlignmentsSupportingAChangeFromTheTargetRead: " << totalNumberOfAlignmentsSupportingAChangeFromTheTargetRead << endl;
//                     cout << "totalNumberOfAlignmentsWithoutGapsSupportingAChangeFromTheTargetRead: " << totalNumberOfAlignmentsWithoutGapsSupportingAChangeFromTheTargetRead << endl;

//                     const double supportingTheTargetReadBaseCountsPercentage = double(coverageOfTheBaseOfTheTargetRead) / double(totalCoverageInThisPosition);
//                     const double supportingTheHetReadBaseCountsPercentage = double(highestCoverageBaseCounts) / double(totalCoverageInThisPosition);

//                     cout << "supportingTheTargetReadBaseCountsPercentage: " << supportingTheTargetReadBaseCountsPercentage << endl;
//                     cout << "supportingTheHetReadBaseCountsPercentage: " << supportingTheHetReadBaseCountsPercentage << endl;

//                     //potentialHetSitesOnOrientedReadId0[positionInRead0] = positionStatsInRead0;
//                     // Check if the key exists, and if not, create it
//                     if (!potentialHetSitesOnOrientedReadId0.contains(positionInRead0)) {
//                         potentialHetSitesOnOrientedReadId0.emplace(positionInRead0, positionStatsInRead0);
//                     }
//                     cout << "positionStatsInRead0.totalNumberOfAlignments: " << -1 << endl;
//                     cout << "positionStatsInRead0.totalNumberOfAlignments: " << potentialHetSitesOnOrientedReadId0[positionInRead0].totalNumberOfAlignments << endl;
...
//                     cout << "positionStatsInRead0.hetBase1: " << potentialHetSitesOnOrientedReadId0[positionInRead0].hetBase1 << endl;
//                     potentialHetSitesOnOrientedReadId0[positionInRead0].hetBase2 = baseCounts[0].second;
//                     cout << "positionStatsInRead0.hetBase2: " << potentialHetSitesOnOrientedReadId0[positionInRead0].hetBase2 << endl;
//                     potentialHetSitesOnOrientedReadId0[positionInRead0].isInHomopolymerRegion = siteIsInHomopolymerRegion;
//                     cout << "positionStatsInRead0.isInHomopolymerRegion: " << potentialHetSitesOnOrientedReadId0[positionInRead0].isInHomopolymerRegion << endl;
...
//                     // Set the orientedReadIds that support the target read base
//                     // Helper function to safely remove reads with specific bases
//                     auto removeReadsWithBases = [&](std::set<OrientedReadId>& result, 
//                                                    const std::set<OrientedReadId>& readsToRemove) {
//                         for (const auto& read : readsToRemove) {
//                             result.erase(read);
//                         }
//                     };

//                     if (positionStatsInRead0.baseOfReadId0 == 0) { // A is the target base
//                         std::set<OrientedReadId> tempResult = coveringOrientedReadIds;
                        
//                         // Remove all reads that have bases other than A
//                         removeReadsWithBases(tempResult, positionStatsInRead0.orientedReadIdsWithC);
//                         removeReadsWithBases(tempResult, positionStatsInRead0.orientedReadIdsWithG);
//                         removeReadsWithBases(tempResult, positionStatsInRead0.orientedReadIdsWithT);
//                         removeReadsWithBases(tempResult, positionStatsInRead0.orientedReadIdsWithGap);
                        
//                         potentialHetSitesOnOrientedReadId0[positionInRead0].hetBase1OrientedReadIds.swap(tempResult);
                    
//                     } else if (positionStatsInRead0.baseOfReadId0 == 1) { // C is the target base
//                         std::set<OrientedReadId> tempResult = coveringOrientedReadIds;
                        
//                         // Remove all reads that have bases other than C
//                         removeReadsWithBases(tempResult, positionStatsInRead0.orientedReadIdsWithA);
//                         removeReadsWithBases(tempResult, positionStatsInRead0.orientedReadIdsWithG);
//                         removeReadsWithBases(tempResult, positionStatsInRead0.orientedReadIdsWithT);
//                         removeReadsWithBases(tempResult, positionStatsInRead0.orientedReadIdsWithGap);
...
//                     } else if (positionStatsInRead0.baseOfReadId0 == 2) { // G is the target base
//                         std::set<OrientedReadId> tempResult = coveringOrientedReadIds;
                        
//                         // Remove all reads that have bases other than G
//                         removeReadsWithBases(tempResult, positionStatsInRead0.orientedReadIdsWithA);
//                         removeReadsWithBases(tempResult, positionStatsInRead0.orientedReadIdsWithC);
//                         removeReadsWithBases(tempResult, positionStatsInRead0.orientedReadIdsWithT);
//                         removeReadsWithBases(tempResult, positionStatsInRead0.orientedReadIdsWithGap);
...
//                     } else if (positionStatsInRead0.baseOfReadId0 == 3) { // T is the target base
//                         std::set<OrientedReadId> tempResult = coveringOrientedReadIds;
                        
//                         // Remove all reads that have bases other than T
//                         removeReadsWithBases(tempResult, positionStatsInRead0.orientedReadIdsWithA);
//                         removeReadsWithBases(tempResult, positionStatsInRead0.orientedReadIdsWithC);
//                         removeReadsWithBases(tempResult, positionStatsInRead0.orientedReadIdsWithG);
//                         removeReadsWithBases(tempResult, positionStatsInRead0.orientedReadIdsWithGap);
...
//                     cout << "positionStatsInRead0.hetBase1OrientedReadIds.size(): " << positionStatsInRead0.hetBase1OrientedReadIds.size() << endl;
//                     cout << "positionStatsInRead0.hetBase2OrientedReadIds.size(): " << positionStatsInRead0.hetBase2OrientedReadIds.size() << endl;


//                     // Set the orientedReadIds that support the other base
//                     if (baseCounts[0].second == 0) { // A is the 'other' base
//                         potentialHetSitesOnOrientedReadId0[positionInRead0].hetBase2OrientedReadIds = positionStatsInRead0.orientedReadIdsWithA;

//                     } else if (baseCounts[0].second == 1) { // C is the 'other' base
//                         potentialHetSitesOnOrientedReadId0[positionInRead0].hetBase2OrientedReadIds = positionStatsInRead0.orientedReadIdsWithC;
                    
                    
//                     } else if (baseCounts[0].second == 2) { // G is the 'other' base
//                         potentialHetSitesOnOrientedReadId0[positionInRead0].hetBase2OrientedReadIds = positionStatsInRead0.orientedReadIdsWithG;
                    
//                     } else if (baseCounts[0].second == 3) { // T is the 'other' base
//                         potentialHetSitesOnOrientedReadId0[positionInRead0].hetBase2OrientedReadIds = positionStatsInRead0.orientedReadIdsWithT;
...
//                     potentialHetSitesOnOrientedReadId0[positionInRead0].totalNumberOfHetBase1 = coverageOfTheBaseOfTheTargetRead;
...
//                         << "  First variant: " << bases[baseCounts[0].second] << " (" << supportingTheTargetReadBaseCountsPercentage * 100 << "%)" << endl
//                         << "  Second variant: " << bases[baseCounts[1].second] << " (" << supportingTheHetReadBaseCountsPercentage * 100 << "%)" << endl;
...
//                 debugOut << "Skipped " << sitesSkippedDueToInsufficientCoverage << " sites due to insufficient coverage" << endl;
//                 readsWithNoHetSites[readId0] = true; // Mark this read as contained
//                 cout << "Thread ID: " << threadId << " Found no potential heterozygous sites in readId " << readId0 << endl;
//                 continue;
//             }
...
//             // if (finalInPhaseOrientedReads.empty()) {
//             //     debugOut << "No in-phase reads found for read ID: " << readId0 << endl;
//             //     readsWithNoHetSites[readId0] = true;
//             //     continue; // Skip to the next readId if no in-phase reads
//             // }
...
//                 //
//                 // TODO: Uncomment this when we have a better way to handle excluded reads.
//                 //
...
//     vector<Site> sites;
//     vector<bool> readsWithNoHetSites(readCount, false);
...
//             // Check if the read has no het sites
//             ReadId currentReadId0 = alignmentData[alignmentId].readIds[0];
//             ReadId currentReadId1 = alignmentData[alignmentId].readIds[1];

//             if(phasingThreadDataReadGraph5->threadReadsWithNoHetSites[threadId][currentReadId0]) {
//                 readsWithNoHetSites[currentReadId0] = true;
//             }
//             if(phasingThreadDataReadGraph5->threadReadsWithNoHetSites[threadId][currentReadId1]) {
//                 readsWithNoHetSites[currentReadId1] = true;
//             }
...
//     // // Add all alignments involving reads with no het sites.
//     // cout << timestamp << "Adding alignments involving reads with no het sites." << endl;
//     // for (ReadId readId = 0; readId < readCount; readId++) {
//     //     if (readsWithNoHetSites[readId]) {
//     //         const Strand strand0 = 0; // Analyze strand 0 arbitrarily, results should be consistent
//     //         const OrientedReadId orientedReadId0(readId, strand0);
//     //         const auto alignmentTable0 = alignmentTable[orientedReadId0.getValue()];
//     //         for (const auto alignmentId : alignmentTable0) {
...
//             const uint64_t readLength0 = reads->getRead(currentOrientedReadId0.getReadId()).baseCount;
//             const uint64_t readLength1 = reads->getRead(currentOrientedReadId1.getReadId()).baseCount;
...
//             // Check for containment.
//             const bool isContained0 = (leftTrimBases1 > leftTrimBases0 && rightTrimBases1 > rightTrimBases0);
//             const bool isContained1 = (leftTrimBases0 < leftTrimBases1 && rightTrimBases0 < rightTrimBases1);
...
//     //     if (site.targetOrientedReadId.getReadId() != 1055 || site.targetOrientedReadId.getReadId() != 366) {
//     //         continue;
//     //     }
...
//                         foundGoodChain = true;
//                         debugOut << "  Site Index: " << siteIndex << " (Pos: " << potentialHetSitesOnTargetOrientedReadId[siteIndex].positionInTargetRead << ") LCG: " << LCG[siteIndex] << endl;
//                     }
//                 }
//                 if (foundGoodChain){
//                     debugOut << endl;
...
//             std::vector<SnpStats> potentialHetSitesOnTargetOrientedReadIdFilteredNoAdjacentSnpsFilteredWellSeparated;
//             const auto& inputSnps = potentialHetSitesOnTargetOrientedReadIdFilteredNoAdjacentSnps;
//             if (!inputSnps.empty()) {
//                 for (size_t i = 0; i < inputSnps.size(); ++i) {
//                     const bool hasPrev = (i > 0);
//                     const bool hasNext = (i < inputSnps.size() - 1);

//                     // Check distance to previous SNP.
//                     const bool prevIsTooClose = hasPrev && (inputSnps[i].positionInTargetRead - inputSnps[i - 1].positionInTargetRead < 32);

//                     // Check distance to next SNP.
//                     const bool nextIsTooClose = hasNext && (inputSnps[i + 1].positionInTargetRead - inputSnps[i].positionInTargetRead < 32);

//                     // Only keep the SNP if it is not too close to either neighbor.
//                     if (!prevIsTooClose && !nextIsTooClose) {
//                         potentialHetSitesOnTargetOrientedReadIdFilteredNoAdjacentSnpsFilteredWellSeparated.push_back(inputSnps[i]);
//                     }
//                 }
//             }
...
//             debugOut << "Filtered vector size after filtering out well separated SNPs: " << potentialHetSitesOnTargetOrientedReadIdFilteredNoAdjacentSnpsFilteredWellSeparated.size() << endl;
//             for (const auto& site : potentialHetSitesOnTargetOrientedReadIdFilteredNoAdjacentSnpsFilteredWellSeparated) {
//                 debugOut << "  Site Index: " << site.positionInTargetRead << " (Pos: " << site.positionInTargetRead << ")" << endl;
//             }
...
//             const auto& informativeSnps = potentialHetSitesOnTargetOrientedReadIdFilteredNoAdjacentSnpsFilteredWellSeparated;
